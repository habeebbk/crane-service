...
class EditTransactionScreen extends StatefulWidget {
  ...
  const EditTransactionScreen({super.key, required this.order});
...
class _EditTransactionScreenState extends State<EditTransactionScreen> {
  ...
  // Dynamic Cost Items - Using Controllers to ensure state persistency
  late List<Map<String, dynamic>> _costControllers;
  ...
  void initState() {
    ...
    _serviceNameController =
        TextEditingController(text: widget.order.serviceName);

    // Initialize Cost Items Controllers
    _costControllers = [];
    if (widget.order.costItems.isNotEmpty) {
      for (var item in widget.order.costItems) {
        _costControllers.add({
          'name': TextEditingController(text: item.name),
          'amount': TextEditingController(text: item.amount.toInt().toString()),
        });
      }
    } else {
      // Legacy support: Create one item with total cost
      _costControllers.add({
        'name': TextEditingController(text: 'Service Cost'),
        'amount':
            TextEditingController(text: widget.order.cost.toInt().toString()),
      });
    }

    _selectedDate = DateFormat('yyyy-MM-dd').parse(widget.order.date);
    _paymentStatus = widget.order.paymentStatus;

    // Check if category exists in list
    if (_categories.contains(widget.order.serviceCategory)) {
      _selectedCategory = widget.order.serviceCategory;
    } else {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      for (var map in _costControllers) {
        costItems.add(CostItem(
          ...
          amount:
              double.tryParse((map['amount'] as TextEditingController).text) ??
                  0,
        ));
      }

      await provider.updateOrderWithDetails(
